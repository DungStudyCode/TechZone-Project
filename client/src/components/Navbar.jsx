...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white shadow-sm sticky top-0 z-50 font-sans border-b border-gray-100">
      <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <Link
              to="/cart"
              className="relative text-gray-500 hover:text-purple-700 transition-colors"
            >
            ...
            {user ? (
                  ...
                  <div className="bg-white rounded-lg shadow-xl border border-gray-100 w-52 overflow-hidden">
                    ...
                    {user.isAdmin && (
                      <>
                        ...
                        {/* 👇 LINK MỚI THÊM VÀO ĐÂY 👇 */}
                        <Link
                          to="/admin/ai-insights"
                          className="block px-4 py-2 text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-700 font-semibold text-purple-900"
                        >
                          🔮 AI Brain Center
                        </Link>
                      </>
              ...
              <Link
                to="/login"
                className="text-purple-900 hover:text-purple-700 p-2 bg-purple-50 rounded-full hover:bg-purple-100 transition-colors"
              >
      ...
      {/* --- MOBILE MENU --- */}
      <div
        className={`lg:hidden bg-white border-t border-gray-100 overflow-hidden transition-all duration-300 ${isMobileMenuOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0"}`}
      >
        <div className="p-4 space-y-4">
          <form onSubmit={submitHandler} className="relative">
            <input
              ...
              className="w-full bg-gray-100 rounded-full py-2 px-4 outline-none"
              onChange={(e) => setKeyword(e.target.value)}
            />
            <button className="absolute right-3 top-2 text-purple-700">
              <FaSearch />
            </button>
          </form>
          <div className="flex flex-col gap-2">
            {navLinks.map((link, index) => (
              <Link
                key={index}
                to={link.path}
                className="py-2 text-gray-700 hover:text-purple-700 font-medium no-underline"
